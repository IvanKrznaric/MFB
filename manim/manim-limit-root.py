...
class Limit1(Scene):
    def construct(self):
        ...
        graf = axes.plot(lambda x : (1/x)**0.5, color = PURE_BLUE, x_range = [0.2, 999, 0.01])
        ...
        table = MathTable([["x", "f(x) = \\frac{1}{\\sqrt{x}}"], ["a", "b"]], include_outer_lines=True).set_color(BLACK)
        ...
        vrijednost_fx = always_redraw(lambda : DecimalNumber((1/x.get_value())**0.5, num_decimal_places = 3).move_to(1.5*LEFT + 2*UP).set_color(BLACK).scale(0.7))
        ...
        self.wait()
        self.play(x.animate.set_value(999), run_time = 7)
        self.wait(2)
