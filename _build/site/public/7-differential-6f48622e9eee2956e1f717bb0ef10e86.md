...
Our task is to find by how much can the market index change so that Marko's function gives accurate values up to $1\%$ of the inital value that is equal to $P(2) = 2744.$ If we denote that $dI$ that displacement of the market index, we would need to solve the following inequality:
\begin{equation*}
\begin{split}
