...
:::{tip} <span style = "color : #228B22"> Definition </span>
:icon:false
Let $A = [a_{ij}]$ be a matrix of order $n \times m$ and let $B = [b_{ij}]$ be a matrix of order $m \times p.$ We define **<span style = "color:green"> matrix multiplication </span>** of matrices $A$ and $B$ as a matrix $C = [c_{ij}]$ of order $n \times p,$ whose elements are given by
