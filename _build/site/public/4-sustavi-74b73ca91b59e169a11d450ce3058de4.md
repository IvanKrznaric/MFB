...
````{solution} P410
:class:dropdown
Again, as [](#P46) shows us, we know that if we know the vector of outputs $Q$ then the problem is easy to solve. Hence, we want to find the total outputs $Q_1$ and $Q_3$ of the first and the third sector.
````