...
:::{prf:remark}
:numbered: false
Suppose that $F$ is an integral of the function $f$. [By definition](#def_integral), that means that $F'(x) = f(x)$. But also the following identities hold:
\begin{equation*}
\begin{split}
...
\begin{equation*}
\int \frac{e^x}{x} -3x^2 \, dx = \int \frac{e^x}{x} \, dx - \int 3x^2 \, dx.
\end{equation*}
So, in order to compute the integral in this question we simply need to compute the two integrals above. The second one is straight-forward, since we can use the formula given in the [table of integrals](#table-intergals):
\begin{equation*}
\int 3x^2 \, dx = x^3.
\end{equation*}
````
