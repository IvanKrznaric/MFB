...
````{solution} P410
:class:dropdown
Again, as we have seen in [](#P46), if we know the vector of outputs $Q$ then it is easy to find the input-output table. Hence, we want to find the total outputs $Q_1$ and $Q_3$ of the first and the third sector.
````