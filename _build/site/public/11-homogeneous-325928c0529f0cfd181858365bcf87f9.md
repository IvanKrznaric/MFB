...
:::{note} Problem 11.8
:icon: false
In a global card payment market, two transaction processing companies $A$ and $B$ are selling their services. Company $A$ is charging the price $p_1$ per transaction and company $B$ is charging $p_2$ per transaction. The demand for the services of the company $B$ is modelled by the function
\begin{equation*}
