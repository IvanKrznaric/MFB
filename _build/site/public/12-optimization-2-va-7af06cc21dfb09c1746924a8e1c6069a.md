...
f_x = 0, \quad f_y = 0
\end{equation*}
4. Use the Hessian matrix to determine the character of the stationary points.
:::
...
:::{note} Problem 12.4
:icon: false
A company produces two goods $A$ and $B$, whose prices are set to $p_1 = 2 , p_2 = 1$ respectively. The total cost function is given by
\begin{equation*}
...
:::{note} ⚙️ Lagrange multiplier method
:icon:false
1. Find the **<span style = "color:green"> Lagrange function </span>**
\begin{equation*}
L(x,y,\lambda) = f(x,y) + \lambda(c-g(x,y))
\end{equation*}
2. Find the stationary points of the Lagrange function $L$
3. Plug-in the stationary points into the Hessian matrix and compute its determinant:
\begin{equation*}
\begin{split}
\det\left( H_{L} \right) > 0 &\implies \text{maximum} \\
\det\left( H_{L} \right) < 0 &\implies \text{minimum} \\
\end{split}
\end{equation*}
:::
...
f(x,y) = -x - y
\end{equation*}
given the constraint $x^2+y^2 = 2.$
:::

:::{note} Problem 12.10
:icon: false
A certain country consumes two imported goods $A$ and $B$, whose imported amounts are $x,y$ respectively. The utility function of consumption of those goods is given by
\begin{equation*}
u(x,y) = \sqrt{xy}.
\end{equation*}
Due to international tensions, a non-linear tariff is imposed such that the total cost to the consumer is modelled by
\begin{equation*}
x^2 + 3xy + 2y^2 = 180.
\end{equation*}
Find the optimal levels of import that maximize the utility function under the tariff-induced cost constraint.
:::