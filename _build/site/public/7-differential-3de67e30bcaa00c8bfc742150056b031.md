...
:icon:false
Consider the function $y(x) = e^x+3.$ This function is given **explicitly** because we are given the formula by which $y$ is defined. However, we can preform the following operations:
\begin{equation*}
\begin{split}
y &= e^x+3 \\
e^x &= y-3 \quad / \ln \\
...
\end{split}
\end{equation*}
and arrive at the expression $x-\ln(y-3) = 0.$ This expression is **implict** form of the function $y$ because we don't immediately know the formula by which the function $y$ is defined, but we can easily express $y$ from that equation and find the explicit form.
:::
...
\begin{equation*}
\label{eqn1}
(r-3)^2 \cdot P + P^4 = e^{4r} \cdot (r-3) + 16.
\end{equation*}
...
Because [the differential approximates the exact change](#differential), we know that
\begin{equation*}
...
\begin{equation*}
\begin{split}
(r-3)^2 \cdot P + P^4 &= e^{4r} \cdot (r-3) + 16 \quad /' \\
2(r-3)P + (r-3)^2 P' + 4P^3P' &= 4e^{4r}(r-3) + e^{4r}
\end{split}
\end{equation*}
...
4P(3)^3P'(3) = e^{12}.
\end{equation*}
Plugging-in $r = 3$ into [the defining equation for $P$](#eqn1), we get 
\begin{equation*}
...
\begin{equation*}
\begin{split}
\lvert P(2+dI) - P(2) \rvert &\leq \frac{5}{100} \cdot P(2) \\
\iff \lvert (14+5dI)^{\sqrt{9+dI}} - 2744 \rvert &\leq 137.2
\end{split}
\end{equation*}
Of course, this inequality is far to complicated for us to solve, so we need to change our approach.

Because [the differential approximates the exact change](#differential), we know that $dP(2) \approx P'(2) \cdot dI.$ Therefore, we have the following approximation
\begin{equation*}
\begin{split}
\lvert P'(2) \cdot dI \rvert \approx \lvert \Delta P(2) \rvert &\leq \frac{1}{100} P(2). \\
\iff \lvert P'(2) \cdot dI \rvert &\leq 137.2
\end{split}
\end{equation*}
...
\begin{equation*}
\begin{split}
\lvert P'(2) \cdot dI \rvert &\leq 137.2 \\
\implies \lvert dI \rvert &\leq \frac{137.2}{P'(2)} = 0.033
\end{split}
\end{equation*}
Therefore, if the market index changes up to $\pm 0.033,$ accuracy of Marko's model will be withing $5\%$ of the actual stock price.
````