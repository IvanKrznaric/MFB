...
\end{split}
\end{equation*}
In order to determine which points is a local minimum and which is a local maximum, we need to compute the second derivative of the function $\Pi$:
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
Plugging-in the stationary points $x = 13$ we get that:
\begin{equation*}
\Pi''(13) &= -4,
\end{equation*}
