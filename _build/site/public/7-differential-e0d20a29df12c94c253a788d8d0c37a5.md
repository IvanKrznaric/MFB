...
````{solution} P79
:class: dropdown
First approach might be the following: find the value of $A$ such that 
\begin{equation*}
R(A) = 1.03 \cdot R(14\hspace{0.1cm}600)
\end{equation*}
Since $R(14\hspace{0.1 cm}600) \approx 1158.81,$ the equation that we need to solve is the following:
\begin{equation*}
...
\end{equation*}
where $A_0 = 14600$ is the initial amount of money being invested in advertising. If the business wants to increase their revenue by $3\%,$ then $\Delta R(A_0) = 0.05R(A_0).$ So, from the approximation above, the additional amount of money that needs to be invested in advertising is 
\begin{equation*}
dA \approx \frac{\Delta R(A_0)}{R'(A_0)} = \frac{0.05R(A_0)}{R'(A_0)} = 
\end{equation*}
````
...
\begin{equation*}
(r-3)^2 \cdot P + P^4 = e^r \cdot (r-3)^2 + 16.
\end{equation*}
