...
:::{tip} <span style = "color : #228B22"> Definition </span>
:icon:false
Let $A = [a_{ij}]$ be a matrix of order $n \times m$ and let $\alpha \in \R$ be a scalar (i.e. a number). **<span style = "color:green"> Multiplication of the matrix $\alpha$ by a scalar </span>** $\alpha$ is a matrix $B = [b_{ij}],$ where 
